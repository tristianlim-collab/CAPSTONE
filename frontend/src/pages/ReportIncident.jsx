...
const INCIDENT_TYPES = [
  ...
  { value: 'CRIME_RELATED', label: 'Crime-Related', icon: '🚨' },
...
const ReportIncident = () => {
  ...
  const uploadPhoto = async (file) => {
    ...
    try {
      ...
      setFormData({ ...formData, photo_url: response.data.data.url });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message = error.response?.data?.message || 'Failed to report incident';
      toast.error(message);
    } finally {
      setSubmitting(false);
    }
